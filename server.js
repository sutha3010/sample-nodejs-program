...
app.post('/submit-form', function(req,res){
  ...
  conn.query(employeeSql , function (err, employeeResult) {
    if (err) {
      if (err.code === 'ER_DUP_ENTRY') {
        // Handle duplicate key error
        console.error('Duplicate key error:', err.sqlMessage);
        res.status(400).send('Duplicate key error: ' + err.sqlMessage);
      } else {
  ...
  console.log('Employee data inserted successfully: ' + employeeResult.insertId);
  ...
  // Insert data into 'events' table
   var eventsSql  = "INSERT INTO event(Event_name, Event_date, Event_time, Event_venue) VALUES ('"+event_name+"','"+event_date+"','"+event_time+"','"+event_venue+"')";    
     conn.query(eventsSql , function (err, eventsResult) {
      if (err) {
        console.error(err);
        res.status(500).send('Error while processing the form');
    ...
    //  });
     console.log('Event data inserted successfully: ' + eventsResult.insertId);
...
conn.query(userSql, [emp_id, pref, food, emp_dept], function (err, userResult) {
  if (err) {
    console.error(err);
    res.status(500).send('Error while inserting data into the database');
    return;
  } 
console.log('User preference data inserted successfully: ' + userResult.insertId);

// Send response after all database operations are complete
res.send('Form filled successfully with ID: ' + employeeResult.insertId);
